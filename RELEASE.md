...
- [ ] `action.yml` — `default:` pin updated to the latest Zenzic core version (`0.22.2`)
- [ ] `package.json` version bumped to `2.8.2`
- [ ] `pyproject.toml` — synchronized with core pin (`zenzic>=0.22.2`)
...
git tag -fa v2 v2.8.2^{} -m "release: v2.8.2"
git push origin v2 --force

# Verification (Atomic Parity Check):
git rev-parse v2^{} v2.8.2^{}
